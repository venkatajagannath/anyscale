...
def update_template_with_memorydb(template_body: str, az_num: int) -> str:
    """
    Add MemoryDB resources and outputs to template body.
    """
    subnet_list = [f"        - !Ref Subnet{i}" for i in range(az_num)]
...
def get_or_create_memorydb(
    ...
    # Update template
    updated_template_body = update_template_with_memorydb(template_body, az_num)
